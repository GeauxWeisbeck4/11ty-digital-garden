...
function launchSearchBox() {
  if (document.querySelector(".search-box")) {
    ...
    searchBox.innerHTML = `
      ...
      <button class="modal-close is-large" aris-label="close" onlick="closeSearchBox()"></button>
    `;
...
function search() {
  ...
  results.forEach((item) => {
    ...
    resultEl.classList.add("panel-block");
    resultEl.innerHTML = `
      <a href="${item.url}">${item.title}</a>
    `;
    document.getElementById("search-results").appendChild(resultEl);
  });
}
